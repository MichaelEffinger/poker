...
class Player
{
	...
	static int calculate_hand(Card[] cards){
		int same_suit = 0;
		...
		{
		case 2:
			int hi = 0 ;// = max(cards[0].rank, cards[1].rank);
			int lo = 0 ;//= min(cards[0].rank, cards[1].rank);
	...
	double calculate_equity(Card[2] holeCards, Card[5]* boardCards){
